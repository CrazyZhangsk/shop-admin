package com.fh.shop.backend.servlet;
...
import com.fh.shop.utils.CookUtil;
...
public class ImageCodeServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*request.getSession().setAttribute(SystemConstants.IMAGECODE, scode);*/

		String cookie = CookieUtil.readCookie(request, SystemConstants.JSESSIONID);
		if (StringUtils.isEmpty(cookie)){
			//在redis开辟空间
			String uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase();
			RedisUtil.set(SystemConstants.SESSION_CODE+uuid,scode);
			RedisUtil.expire(SystemConstants.SESSION_CODE+uuid,5*60);
			//分配到客户端
			CookieUtil.writeCookie(response, SystemConstants.JSESSIONID, uuid, SystemConstants.SESSION_DOMAIN, -2);
		}else {
			RedisUtil.set(SystemConstants.SESSION_CODE+cookie, scode);
			RedisUtil.expire(SystemConstants.SESSION_CODE+cookie,5*60);
		}
